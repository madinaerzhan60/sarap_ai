...
const logo = () => `<span class="brand"><svg class="brand-mark" viewBox="0 0 48 48" fill="none" aria-hidden="true"><rect width="48" height="48" rx="15" fill="#1E8F85"/><path d="M34 13H21a8 8 0 0 0 0 16h5a4 4 0 0 0 0-8h-5a4 4 0 0 1 0-8h13M14 35h13a8 8 0 0 0 0-16" stroke="white" stroke-width="3" stroke-linecap="round"/><circle cx="35" cy="35" r="3.5" fill="#E2AD5A"/></svg><span>SARAP<small>REPUTATION INTELLIGENCE</small></span></span>`;
...
function landing() {
  return `<main class="landing-shell">
    ...
    <section class="hero"><div class="eyebrow">AI reputation early-warning for Kazakhstan</div><h1>Turn customer <span>feedback</span> into decisions.</h1><p>SARAP monitors reviews, social conversations, media and web mentions about your business and alerts you when something needs attention.</p><div class="hero-actions"><button class="btn btn-primary" data-action="register">Start monitoring</button><button class="btn btn-secondary" data-action="demo">View demo</button></div></section>
    <section class="signal-flow glass" aria-label="SARAP product flow"><div class="source-cloud"><span class="source-chip">${icons.review}2GIS</span><span class="source-chip">${icons.review}Google</span><span class="source-chip">${icons.mentions}Instagram</span><span class="source-chip">${icons.news}News</span><span class="source-chip">${icons.web}Web</span></div><span class="flow-arrow">→</span><div class="flow-node">${icons.radar}<strong>SARAP</strong><small class="muted">filters signal from noise</small></div><span class="flow-arrow">→</span><div class="flow-node">${icons.alerts}<strong>AI risk alert</strong><small class="muted">evidence + next action</small></div></section>
    <section class="features"><article class="feature glass"><div class="feature-icon">${icons.radar}</div><h3>Reputation Radar</h3><p>Monitor reviews, social mentions, news and web discussions from one calm workspace.</p></article><article class="feature glass"><div class="feature-icon">${icons.alerts}</div><h3>Critical Alerts</h3><p>Get notified when potentially damaging feedback needs a human response.</p></article><article class="feature glass"><div class="feature-icon">${icons.analytics}</div><h3>Aspect Intelligence</h3><p>See whether complaints concern service, staff, product, delivery, price or waiting time.</p></article><article class="feature glass"><div class="feature-icon">${icons.mentions}</div><h3>Kazakh + Russian</h3><p>Understand Kazakh, Russian, English and natural mixed-language customer feedback.</p></article></section>
...
function mapStoredSource(source) {
  ...
  return {id:source.id,dbId:source.id,backendId:source.id,name:source.source,kind,method:kind==='Imported'?'Import':labels[source.collection_mode]||'Auto',status:source.status||'Active',state:source.error_message?'high':'live',description:source.source_url||'Credentials are configured separately',sourceUrl:source.source_url||'',last:source.last_checked_at?new Date(source.last_checked_at).toLocaleString():'—',next:source.next_check_at?new Date(source.next_check_at).toLocaleString():'—',items:0,errors:source.error_message?1:0};
}
...
async function onboardingSubmit(form) {
  ...
  if(state.onboardingStep<3) state.onboardingStep++; else {
    ...
    if(data.twoGis){
      state.sources[0]={...state.sources[0],description:`Monitoring ${data.twoGis}`,sourceUrl:data.twoGis};
      if(!state.session?.demo){
        try{const saved=await createBackendSource({name:'2GIS',method:'auto',url:data.twoGis});if(saved)state.sources=[mapStoredSource(saved)];}catch(error){toast('Workspace saved, source pending',error.message);}
      }
    }
...
function addSourceModal() {
  modal(`<div class="modal-head"><div><h2>Connect a source</h2><p>Choose how SARAP should collect this source.</p></div><button class="close" data-action="close-modal">×</button></div><form data-form="source"><div class="field"><label>Source</label><select name="name"><option>2GIS</option><option>Yandex Maps</option><option>Google Business</option><option>Instagram</option><option>Threads</option><option>Telegram</option><option>YouTube</option><option>Website / RSS</option><option>CSV Import</option></select></div><div class="field"><label>Collection strategy</label><select name="method"><option value="auto">Auto — official API, then URL fallback</option><option value="api">Official API only</option><option value="scraper">Public URL scraper only</option><option value="import">Manual import</option></select></div><div class="field"><label>Public business URL</label><input name="url" type="url" placeholder="https://2gis.kz/almaty/firm/…"></div><p class="form-note">Auto mode uses an official connector whenever credentials are available. URL collection reads only permitted public pages and does not bypass login, CAPTCHA or source blocking.</p><div class="form-actions"><span></span><button class="btn btn-primary">Add source</button></div></form>`);
}
...
document.addEventListener('submit', async e => {
  ...
  if(kind==='source'){const d=fieldData(form);if(d.method==='scraper'&&!d.url){toast('URL required','Scraper mode needs a public source URL.');return;}const duplicate=state.sources.some(s=>s.name===d.name&&s.method===d.method);if(duplicate){toast('Already connected','This source and collection strategy already exist.');return;}try{if(state.session?.demo){const labels={auto:'Auto · API preferred',api:'API only',scraper:'URL scraper',import:'Import'};state.sources.push({id:crypto.randomUUID(),name:d.name,kind:d.method==='api'?'Official':d.method==='import'?'Imported':'Monitored',method:labels[d.method],status:'Active',state:'live',description:d.url||'Demo source',sourceUrl:d.url||'',last:'—',next:'15 min',items:0,errors:0});}else{const backend=await createBackendSource(d);state.sources.push(mapStoredSource(backend));}}catch(error){toast('Could not save source',error.message);return;}saveState();closeModal();render();toast('Source added',`${d.name} is saved and ready to test.`);}
